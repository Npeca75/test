...
function srchipconn($modnr, $conn, $file)
{
    ...
    foreach ($lines as $line_num => $line) {
        ...
        if ($line == "interface ip input $modnr/$conn") {
            $flint = 1;
        }
...
//        if (strpos ($line, "interface qam output $modnr/1.$submnr") !== false)  { $flint=1; }
        if ($line == "interface qam output $modnr/1.$submnr") {
            $flint = 1;
        }
        ...
        if ($flout and strpos($line, 'manual-name') !== false) {
            ...
            if ($tmp[0] != 'no') {
                $name = trim(str_replace('manual-name', '', $line));
            }
            $flname = 1;
        }
...
while ($r = mysqli_fetch_assoc($q)) {
    ...
    if ($qm != '') {
        mysqli_query($db, "update OutQamServ $qm where lum='$lum' and module='$module' and red0='$red0'");
    }
}
