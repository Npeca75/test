...
while ($r=mysqli_fetch_assoc($q)) {
    ...
    $sidin = $r2['sidin'];
    if (!$nout) {$flred[$cnt]=1;} else {$flred[$cnt]=0;}
...
$nip = readline("New IP: ? ");
if (!filter_var($nip, FILTER_VALIDATE_IP)) { exit; }

$nport = readline("New Port: [5000] ? ");
if ( $nport=="" ) { $nport=5000; }
if ( intval($nport) < 1000 or intval($nport) > 50000) { exit; }

$tmp = readline("New Vlan: [$v] ? ");
if ($tmp=="") { $nvlan=trim($v); } else { $nvlan=trim($tmp); }

$tmp = readline("Input Name: [$nin] ? ");
if ($tmp=="") { $nin=trim($nin); } else { $nin=trim($tmp); }

$tmp = readline("Input SID: [1000] ? ");
if ($tmp=="") { $sid="1000"; } else { $sid=intval($tmp); }

$tmp = readline("Output Name: [$nout] ? ");
if ($tmp=="") { $nout=trim($nout); } else { $nout=trim($tmp); }
$nout="\"" . $nout. " \"";
...
do {
    ...
    if (_write($sw[$cnt],$h, "end", $prompt[$cnt]) == 1) { unset($h); $cnt--; continue; } //test write
    ...
    _write($sw[$cnt],$h, "", $prompt[$cnt]); sleep(2); //blank write
    _write($sw[$cnt],$h, "no shutdown", $prompt[$cnt]); //turn ON input
    _write($sw[$cnt],$h, "exit", $prompt[$cnt]); sleep(2); //interface exit
    ...
    _write($sw[$cnt],$h, "exit", ""); sleep(2);  //main exit
